...
<body>
    ...
    <div id="content" class="content" style="margin-bottom: 70px !important;">
        <div class="container">
            ...
            <div class="row">
                <h5 class="card-title text-white text-center"><i class="fas fa-chalkboard-teacher m-2"></i>Teachers Info.</h5>
                <table class="table table-hover">
                    ...
                    <tbody>
                        @forelse ($teacher as $eachteacher)
                        
                        @php
                            $exam_course_ids = App\Exam::select('xm_course')->where('teacher_id',$eachteacher->id)->distinct('xm_course')->get();
                        @endphp
                       
                        <tr>
                          <th scope="row">{{ $loop->iteration }}</th>
                          <td>{{ $eachteacher->name??null }}</td>
                          <td>{{$eachteacher->email??null}}</td>
                          <td>
                              <table>
                                  @forelse ($exam_course_ids as $item)
                                  @php
                                       $course_name = App\Course::select('course_title')->where('id',$item->xm_course)->first();
                                  @endphp
                                      <tr>
                                          <td>
